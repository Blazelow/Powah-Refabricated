...
public class PowahJeiPlugin implements IModPlugin {
    ...
    public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
        registration.addRecipeCatalyst(Blcks.ENERGIZING_ORB, JeiEnergizingCategory.TYPE);
        Blcks.ENERGIZING_ROD.getAll().forEach(block -> registration.addRecipeCatalyst(block, JeiEnergizingCategory.TYPE));
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        registration.addRecipes(JeiEnergizingCategory.TYPE, JeiEnergizingCategory.getAllRecipes());
        ...
        if (Powah.config().general.player_aerial_pearl)
            registration.addIngredientInfo(Itms.PLAYER_AERIAL_PEARL, Component.translatable("jei.powah.player_aerial_pearl"));
        if (Powah.config().general.dimensional_binding_card)
            registration.addIngredientInfo(Itms.BINDING_CARD_DIM, Component.translatable("jei.powah.binding_card_dim"));
        if (Powah.config().general.lens_of_ender)
            registration.addIngredientInfo(Itms.LENS_OF_ENDER, Component.translatable("jei.powah.lens_of_ender"));
    }
